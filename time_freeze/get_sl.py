...
usage- python get_sl.py  -t TWDIS_06685 -c TWDIS_09536 -s 2 -l label_file -o out_file
'''
import os
...
class GetSecondLayer():
    ...
    def get_secondlayer_relation(self):
        ...
        self.mydict['concept_ids_subject'] = ",".join(self.set_of_concepts)
        self.mydict['vocab_ids'] = "ONT1006"
        results = self.session.post(f"{self.base_url}conceptset/{self.search_url}/", self.mydict)
    ...
    def get_intranetwork(self):
        ...
        self.mydict['concept_ids_subject'] = ",".join(set_of_concepts2)
        self.mydict['concept_ids_object'] = ",".join(set_of_concepts2)
        self.mydict['vocab_ids'] = "ONT1006"
        results = self.session.post(
            f"{self.base_url}conceptset/{self.search_url}/", self.mydict)
        results = results.json()
